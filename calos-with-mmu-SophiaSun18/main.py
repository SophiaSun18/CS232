...
<src> and <dst> can be a register name, ba <value>, or *<value>.
...
26: sub 1 reg 1       # loop: subtract 1 from val2
...
class Monitor:
    ...
    def _handle_data_label(self, addr, line, pcb):
        if len(line.split()) != 2:
            raise ValueError("Illegal format: __data: must be followed by valid value.")
        num_bytes = int(line.split()[1])
        high_mem = addr + num_bytes
        pcb.set_high_mem(high_mem)
        if self._debug:
            print("__data read as", num_bytes, "and stored high memeory location as", high_mem)
